 ...
 *  first), plus Pending, always shown even at zero members, so the strip
 *  always reflects exactly what's configured on the Configuration page.
 *  Unranked is deliberately excluded from this strip (still a real band
 *  elsewhere — filter pills, table rows, the member drawer — just not its
 *  own status card here). Each card's headline number is the *average*
 *  latestScore of the people in that band, not how many of them there are —
 *  the member count moves to subtext instead. Pending has no score to
 *  average, so it keeps showing its member count. */
